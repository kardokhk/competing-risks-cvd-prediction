...
3. **Impute, fit and evaluate.** `src/v2/run_all.sh` gives the order of the numbered scripts,
   ...
   submitted.
4. **Recompute the closed form.** `src/closed_form/cf2_decomposition.py`, `cf4_validation.py` and
   `cf5_nomogram.py` derive the identity, decompose the naive-versus-competing difference, validate
   it on 90 strata and build the nomogram, writing `results/closed_form/`. They run in minutes on
   the Python stack alone and reproduce every file in that directory byte-identically from what is
   in this repository. `cf1_cause2_fit.R` and `cf3_boot_refit.R` refit the cause-specific Cox model
   and are the only steps here that need R; their outputs are provided, so you can skip them.
...
Steps 4 and 5 read only files that are in this repository, so the closed-form decomposition, all
three tables and all twelve figures can be reproduced without rerunning any model. That was checked
on 2026-09-06 from a fresh clone: all twelve `_data.csv` files, the three table CSVs and every file
in `results/closed_form/` came back byte-identical. Steps 1 to 3 reproduce the fitted predictions
themselves and need the two environments and a machine with the memory and time noted above.

`data/processed/model_matrices/full.parquet`, `results/closed_form/cause2_predictions.parquet` and
the four per-subject prediction files in `results/predictions/` are included because the closed-form
steps read them. The remaining prediction and bootstrap files are not: they are large and are
regenerated by steps 1 to 3.
